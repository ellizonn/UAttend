...
public class UI_lezione {
	 ...
	 * RF15 Modifica lezione
	 * @author Manganoni Mattia Frassinelli Marco Omar
	 * funzione che esegue la modifica della lezione 
	 * @param l
	 * @return
	 */
	...
	public lezione modifica_lezione(lezione l) {
		...
		// SI modificabile
			...
			lezione lNew = new lezione(l.nome_corso, l.cognome_docente, l.anno, l.numero_aula, l.posti_disponibili,
					l.giorno, l.ora_inizio, l.ora_fine);
			sc = new Scanner(System.in);
			String option = "";
			while (true) {
	...
	private lezione setLezione(String option, lezione l, lezione lNew) {
		...
		case "oraIn":
				lNew = g_lez.setOraInizio(lNew);
			return lNew;
		// imposto ora di fine lezione
		case "oraFin":
			
				lNew = g_lez.setOraFine(lNew);
			return lNew;
		// imposto la data della lezione
		case "data":
				lNew = g_lez.setData(lNew);
			return lNew;
		case "aula":
			aula AulaNew = avvia_seleziona_aula_libera(lNew.giorno, lNew.ora_inizio, lNew.ora_fine);
				lNew = g_lez.setAula(lNew, AulaNew);
			return lNew;
		case "salva":
			if (mostra_messaggio_conferma()) {
				if (g_lez.salvaModifiche(l, lNew)) {
					System.out.println("Lezione modificata salvata con successo");
					System.out.println("creazione avviso per la modifica lezione");
					crea_nuovo_avviso(l, lNew); //chiamata del sistema di creazione di un nuovo avviso
				} else
					System.out.println("Attenzione: lezioni uguali, lezione non salvata");
			}
			return lNew;
	 ...
	 * RF15 Modifica lezione
	 * 
	 * funzione che inizia la modifica della lezione
	 * @author Manganoni Mattia Frassinelli Marco Omar RF15 funzione che riceve la
